...
def generate_card_svg(prs_count, issues_count):
    # Expanded vertical layout with transparent card and adaptive color scheme
    return f"""<svg width="300" height="380" viewBox="0 0 300 380" xmlns="http://www.w3.org/2000/svg" style="background: transparent; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
  <defs>
    ...
    <filter id="shadowGreen" x="-20%" y="-20%" width="140%" height="140%">
      <feDropShadow dx="0" dy="2" stdDeviation="3" flood-color="#059669" flood-opacity="0.2" />
    </filter>
  </defs>
  ...
  <!-- Transparent Card Background (allowing page background to show through) -->
  <rect x="1" y="1" width="298" height="378" rx="16" fill="none" stroke="none" />
  ...
  <g transform="translate(150, 130)">
    <!-- Background track circle (r=50, stroke-dasharray="235.6 78.6" for 270-degree cutout) -->
    <circle cx="0" cy="0" r="50" class="prs-track" stroke-width="8.5" stroke-linecap="round" fill="none" 
            stroke-dasharray="235.6 78.6" transform="rotate(135)" />
    <!-- Foreground Gauge -->
    <circle cx="0" cy="0" r="50" stroke="url(#purpleGrad)" stroke-width="8.5" stroke-linecap="round" fill="none" 
            stroke-dasharray="235.6 78.6" transform="rotate(135)" filter="url(#shadowPurple)" />
    <!-- Centered Number -->
    <text x="0" y="-8" font-size="24" font-weight="bold" class="prs-num" text-anchor="middle" dominant-baseline="middle">{prs_count}</text>
    <!-- Labels inside circle with optimized vertical space -->
    <text x="0" y="18" font-size="10.5" font-weight="800" class="prs-lbl1" text-anchor="middle">FOSS PRs</text>
    <text x="0" y="30" font-size="9" font-weight="600" class="prs-lbl2" text-anchor="middle">MERGED</text>
  </g>

  <!-- Gauge 2: FOSS Issues Authored (Green Theme) -->
  <g transform="translate(150, 275)">
    <!-- Background track circle -->
    <circle cx="0" cy="0" r="50" class="iss-track" stroke-width="8.5" stroke-linecap="round" fill="none" 
            stroke-dasharray="235.6 78.6" transform="rotate(135)" />
    <!-- Foreground Gauge -->
    <circle cx="0" cy="0" r="50" stroke="url(#greenGrad)" stroke-width="8.5" stroke-linecap="round" fill="none" 
            stroke-dasharray="235.6 78.6" transform="rotate(135)" filter="url(#shadowGreen)" />
    <!-- Centered Number -->
    <text x="0" y="-8" font-size="24" font-weight="bold" class="iss-num" text-anchor="middle" dominant-baseline="middle">{issues_count}</text>
    <!-- Labels inside circle with optimized vertical space -->
    <text x="0" y="18" font-size="10.5" font-weight="800" class="iss-lbl1" text-anchor="middle">FOSS ISSUES</text>
    <text x="0" y="30" font-size="9" font-weight="600" class="iss-lbl2" text-anchor="middle">AUTHORED</text>
  </g>
</svg>"""
